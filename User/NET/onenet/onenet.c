...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *json,*json_value,*json_value2;
	
	type = MQTT_UnPacketRecv(cmd);  //mqqt解包
	switch(type)
	{
...
//////LED
//					json_value = cJSON_GetObjectItem(json, "LED"); // 获取 JSON 对象中名为 "target" 的键对应的值
//					if(json_value != NULL) 
//					{ // 如果获取的值不为空
//						UsartPrintf(USART_DEBUG, "json_string: [%s]\n", json_value->string); // 打印获取到的值的字符串形式
//						UsartPrintf(USART_DEBUG, "json_value: [%s]\n", json_value->valuestring); // 打印获取到的值的字符串形式
//						if (strstr(json_value->valuestring, "true") != NULL) 
//							{ // 如果值中包含 "LED"

//									beep_flag=1;
//									UsartPrintf(USART_DEBUG, "led已打开\r\n");
//								} 
//								else 
//								{
//									beep_flag=0;
//									UsartPrintf(USART_DEBUG, "LED已关闭\r\n");
//								}
//					} 
//					else 
//					{
//							UsartPrintf(USART_DEBUG, "Error: target key not found\n"); // 如果获取的值为空，则打印错误信息：未找到 "target" 键
//					}
///////BEEP					
					json_value2 = cJSON_GetObjectItem(json, "BEEP"); // 获取 JSON 对象中名为 "target" 的键对应的值
					if(json_value2 != NULL) 
					{ // 如果获取的值不为空
						UsartPrintf(USART_DEBUG, "json_string: [%s]\n", json_value2->string); // 打印获取到的值的字符串形式
						UsartPrintf(USART_DEBUG, "json_value: [%s]\n", json_value2->valuestring); // 打印获取到的值的字符串形式
						if (strstr(json_value2->valuestring, "true") != NULL) 
							{ // 如果值中包含 "BEEP"

								beep_flag=1;
									UsartPrintf(USART_DEBUG, "BEEP已打开\r\n");
								} 
								else 
								{
								beep_flag=0;
									UsartPrintf(USART_DEBUG, "BEEP已关闭\r\n");
								}
					} 
					else 
					{
